...
/*** TO BE DONE START ***/
	...
	if (impl->pathname==NULL)
		impl->pathname=my_strdup(vt_lookup(vt, HOME));
	

	if(chdir(impl->pathname)==-1)
		printf("chdir: %s :No such file or directory\n",impl->pathname);
	
	char *cwd;
	if ( (cwd=getcwd(NULL,0)) == NULL ) // ricavo il path corrente
		fail_errno("Error getcwd");
	vt_set_value(vt, PWD, cwd);
	free(cwd);//libero la memoria allocata da getcwd
...
/*** TO BE DONE START ***/
	...
	char* subPath; //path elaborato dalla strchr
	char* newPath; //path da analizzare
	const char* likePath=path; //contiene i vari path
	do
	{
		subPath=strchr(likePath,DIR_SEPARATOR);
		
		//se null devo anazillare ancora l'ultima stringa in subPath
		if(subPath==NULL)
		{
			newPath=my_malloc(sizeof(char)*(strlen(likePath)+strlen(name)+2));
			strcpy(newPath,likePath);
		}
		else
		{
			size_t lentNewPath= subPath - likePath;
			newPath=my_malloc(sizeof(char)*(lentNewPath+strlen(name)+2));
			strncpy(newPath,likePath,lentNewPath);
			likePath=++subPath;
			newPath[lentNewPath] = '\0';
		}
		//costruisco la stringa da analizzare
		strcat(newPath,"/");
		strcat(newPath,name);
		//controllo se esiste
...
/*** TO BE DONE START ***/
		//while(1);
		execve(executable,impl->args,sh_get_environment(sh));
		fail_errno("Error execve");
...
/*** TO BE DONE START ***/
	...
	if(pipe(pipes)==-1)
		fail_errno("Error pipe");

	for (i = 0; i < 2 ; i++)
		if (fcntl(pipes[i],F_SETFD,FD_CLOEXEC)==-1)
			fail_errno("Error fntcl");
	
	//esploro il ramo sinistro per cercare il comando da eseguire
	impl->left_cmd->execute(impl->left_cmd, sh, in_redir,  pipes[1]);
	//esploro il ramo destro per eseguire il comando	
	impl->right_cmd->execute(impl->right_cmd, sh, pipes[0],  out_redir);
