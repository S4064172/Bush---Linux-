...
/*** TO BE DONE START ***/
		...
		env = my_malloc(sizeof(char*)*(this->len+1)); 
		{	
			int i;	
			for (i=0 ; i < this->len ; i++)
			{
				env[i]=my_malloc(sizeof(char)*strlen(this->vars[i].name)+strlen(this->vars[i].value)+2);
				strcpy(env[i],this->vars[i].name);
				strcat(env[i],"=");
				strcat(env[i],this->vars[i].value);
			}
			env[i]=NULL;
