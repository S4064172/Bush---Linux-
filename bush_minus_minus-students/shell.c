...
/*** TO BE DONE START ***/
		...
		if (envp==NULL)
			return;
		int i;
		for(i=0 ; envp[i]!=NULL ; i++)//libera i char* allocati in precedenza
			free(envp[i]);
		free(envp);//libera i char ** allocati in precedenza
...
void wait_for_termination_of_children()
{
	 ...
	 * (cioè se l'exit_status è diverso da 0; usare WIFEXITED
	 * e WEXITSTATUS per vedere se un processo e` terminato con exit
	 * ed estrarne lo stato) */
/*** TO BE DONE START ***/
//lsa | grep u | grep t | grep ils | greop
	int status;
	while(1)
	{	
			wait(&status);
			if(ECHILD==errno)
				return;
			//se == true è terminato un filgio 
			// ENOENT 2 /* No such file or directory */
			// se non faccio il secondo controllo, a volte rimane 
			//appesa la bush 
			if(WIFEXITED(status) && ENOENT!=errno )
			{
